"""
This is the state representation code used for prior set three and four (the continuous priors).
The only difference is that this implements the new architecture
"""
...
class StateRepresentation(object):
    ...
    def __init__(self, obs_dim, seed=1, modelname="model", continuelearning=False, usingros=False, state_dim=5):
        ...
        self.memory = ReplayBuffer(self.obs_count, act_dim=2, size=3e4)

        # DEFINE THE FUNCTIONS REQUIRED FOR TRAINING
        self.graph = tf.Graph()
        with self.graph.as_default():
            # Define the placeholders needed here
            ...
            self.act_3 = tf.placeholder(tf.float32, shape=[None, 2], name='action_3')
            self.rew_1 = tf.placeholder(tf.float32, shape=[None, 1], name='reward_1')
            self.rew_3 = tf.placeholder(tf.float32, shape=[None, 1], name='reward_3')
            self.drew_1 = tf.placeholder(tf.float32, shape=[None, 1], name='dreward_1')
            self.drew_3 = tf.placeholder(tf.float32, shape=[None, 1], name='dreward_3')

            self.is_training = tf.placeholder(tf.bool, shape=[], name="train_cond")

            # Define the neural network architecture and its output
            self.nn = snt.Module(self.custom_build, name='SRL_Network')
            ...
            self.temp_coh_loss = self.temporal_coherence_prior(self.state_delta, self.act_1)
            self.caus_loss = self.causality_prior(self.state_1, self.state_shuff, self.act_1, self.act_3, self.rew_1, self.rew_3, self.beta)
            self.prop_loss = self.proportionality_prior(self.state_delta, self.state_delt_shuff, self.act_1, self.act_3,
                                                        self.beta)
            self.repeat_loss = self.repeatability_prior(self.state_1, self.state_shuff, self.state_delta,
                                                        self.state_delt_shuff, self.act_1, self.act_3, self.beta)

            graph_regularizers = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
            total_regularization_loss = tf.reduce_sum(graph_regularizers)

            self.losses = [0, self.temp_coh_loss, 2 * self.caus_loss, self.prop_loss, self.repeat_loss,
    ...
    def custom_build(self, inputs, is_training, l2_reg=0.001):
        ...
        # Camera layer
        camera_inputs = snt.SliceByDim(dims=[1], begin=[0], size=[self.obs_dim[0] * self.obs_dim[1] * 3])(inputs)
        ...
        camera_norm1 = tf.layers.batch_normalization(camera_conv1, training=is_training)

        camera_conv2 = tf.layers.conv2d(camera_norm1, filters=64, kernel_size=3, strides=1, padding='valid',
                                        activation=tf.nn.relu, kernel_regularizer=regularizers["w"],
                                        kernel_initializer=initializers["w"])
        camera_norm2 = tf.layers.batch_normalization(camera_conv2, training=is_training)

        camera_flatten = snt.BatchFlatten()(camera_norm2)
        ...
        laser_norm1 = tf.layers.batch_normalization(laser_conv1, training=is_training)

        laser_flatten = snt.BatchFlatten()(laser_norm1)
        laser_dense1 = tf.layers.dense(laser_flatten, 64, activation=tf.nn.relu, kernel_regularizer=regularizers["w"],
                                       kernel_initializer=initializers["w"])

        # Merged layers
        merge_input = tf.concat(values=[camera_dense1, laser_dense1], axis=1)  # 128
        ...
        merge_dense = tf.layers.dense(merge_dense2, self.state_dim, activation=None,
        ...
        merge_noise = lambda x: tf.cond(self.is_training, lambda: x + tf.random_normal(shape=tf.shape(x), stddev=1e-6),
                                        lambda: x)
        return merge_noise(merge_dense)
    ...
    def learn(self):
        with tqdm(total=self.memory.size) as pbar:
            ...
            for _ in range(nr_batches):
                ...
                dr1 = np.abs(b1[1].get('rews') - b1[0].get('rews'))[:l]
                dr2 = np.abs(b2[1].get('rews') - b2[0].get('rews'))[:l]
                ...
                feed_dict = {self.obs_1: b1[0].get('obs')[:l], self.obs_2: b1[1].get('obs')[:l],
                             ...
                             self.act_3: b2[0].get('acts')[:l],
                             self.rew_1: np.reshape(b1[1].get('rews')[:l], (-1, 1)),
                             self.rew_3: np.reshape(b2[1].get('rews')[:l], (-1, 1)),
                             self.drew_1: np.reshape(dr1, (-1, 1)),
                             self.drew_3: np.reshape(dr2, (-1, 1)),
    ...
    def temporal_coherence_prior(self, s_d, a1):
        return tf.reduce_mean(tf.math.exp(-2 * tf.norm(a1, ord=2, axis=1)) * tf.norm(s_d, ord=2, axis=1) ** 2)

    def causality_prior(self, s1, s2, a1, a2, r1, r2, beta):
        """ e ^ -||s1-s2||^2 * e ^ -beta*||a1-a2||^2 * ||r1-r2|| """
...
if __name__ == '__main__':
    ...
    # Load saved observations from memory
    # use a relative path so that we don't need to change the path on every PC
    parentDirectory = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    folder = os.path.join(parentDirectory, 'training_results/trainingdata/results.pkl')
    data = load_pickle(folder)

    print('Loaded {} data points'.format(len(data)))

    srl = StateRepresentation([32, 24, 40], 5)
    ...
    for epoch in range(epochs):
        ...
        losses_mat.append(losses)
    plotter.plot_priors_history(np.array(losses_mat))
    ...
    st, _ = srl.get_memory_states()

    # visualise the results
    print('Plotting the results')

    plotter.plot_all_srl(st, srl.memory.get_all_samples().get('rews'), lg.logDict, trainingcycle=2, save=True,
                         size=29000)
    plotter.plot_priors_history(np.array(losses_mat), name='Loss history SRL network')

    datafolder = "../training_results"
    plotter.saveMultipage(datafolder + "/srl_continuous.pdf")

    # -----------------start PCA analysis after training--------------------
    data = load_pickle(folder)[:14999]

    srl.memory.remove_all()  # clear memory to start again

    # reload data into the memory
    gt = []
    for d in data:
        srl.remember(reshape_observation(d[0]), d[1], d[2], d[3], d[4])
        gt.append(d[0][40:45])
    ...
    st, _ = srl.get_memory_states()
    print(st.shape, srl.memory.size)
    pca = PCA(n_components=4)
    states_pca = np.array(pca.fit_transform(np.array(st)))
    gt = np.array(gt)
    print(gt.shape, states_pca.shape, gt[0, :].shape)
    cor_mat = np.zeros((4, 3))
    for i in range(4):
        for j in range(3):
            c = np.corrcoef(normalize(states_pca[:, i]), normalize(gt[:, j]))
            cor_mat[i, j] = c[0, 1]
    print('TRAINING DATA:')
    print(cor_mat)
